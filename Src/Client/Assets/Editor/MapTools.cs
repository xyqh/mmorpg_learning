...
public class MapTools : MonoBehaviour {
    ...
    public static void ExportTeleporters()
    {
        ...
        foreach(var map in DataManager.Instance.IMaps)
        {
            ...
            {
                if (!DataManager.Instance.ITeleporters.ContainsKey(teleporter.ID))
                {
                    EditorUtility.DisplayDialog("错误", string.Format("地图:{0} 中配置的 Teleporter:[{1}] 中不存在", map.Value.Resource, teleporter.ID), "确定");
                    return;
                }

                TeleporterDefine def = DataManager.Instance.ITeleporters[teleporter.ID];
    ...
    public static void ExportSpawnPoints()
    {
        ...
        if(DataManager.Instance.ISpawnPoints == null){
            DataManager.Instance.ISpawnPoints = new Dictionary<int, Dictionary<int, SpawnPointDefine>>();
        }
        ...
        foreach (var map in DataManager.Instance.IMaps)
        {
            ...
            if (!DataManager.Instance.ISpawnPoints.ContainsKey(map.Value.ID))
            {
                DataManager.Instance.ISpawnPoints[map.Value.ID] = new Dictionary<int, SpawnPointDefine>();
            }

            foreach (var spawnPoint in spawnPoints)
            {
                if (!DataManager.Instance.ISpawnPoints[map.Value.ID].ContainsKey(spawnPoint.ID))
                {
                    DataManager.Instance.ISpawnPoints[map.Value.ID][spawnPoint.ID] = new SpawnPointDefine();
                }
                SpawnPointDefine def = DataManager.Instance.ISpawnPoints[map.Value.ID][spawnPoint.ID];
