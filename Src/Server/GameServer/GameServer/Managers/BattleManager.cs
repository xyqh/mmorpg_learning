...
namespace GameServer.Managers
{
    class BattleManager : Singleton<BattleManager>
    {
        ...
        {
            Log.InfoFormat("BattleManager.ProcessBattleMessage: skill :{0} caster:{1} target:{2} position:{3}", request.castInfo.skillId, request.castInfo.casterId, request.castInfo.targetId, request.castInfo.Position.ToString());
